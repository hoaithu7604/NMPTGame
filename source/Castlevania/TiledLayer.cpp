...
#include "Camera.h"
CTiledLayer::CTiledLayer(json root)
{
	id = root[TILED_LAYER_ID].get<int>();
	name = root[TILED_LAYER_NAME].get<string>();
	type = root[TILED_LAYER_TYPE].get<string>();	

	if (type == TILED_LAYER_TYPE_OBJECTGROUP) 
	{
		json objectsRoot = root[TILED_LAYER_OBJECTS];
		for (json::iterator it = objectsRoot.begin(); it != objectsRoot.end(); it++)
		{
			LPTILEDOBJECT object = new CTiledObject(*it);
			objects.push_back(object);
		}
	}
	else if (type == TILED_LAYER_TYPE_TILELAYER)
	{
...
void CTiledLayer::Draw(LPTILEDTILESET tileset)
{
